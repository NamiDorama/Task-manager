	...
	var Tasks = function () {
		...
		_createClass(Tasks, [{
			...
			value: function init() {
				...
				this.checkToken(self).then(this.getTasks).then(this.sortTasks).then(this.render).then(this.changePageColors).then(this.eventChangeColor).then(this.editTaskWindow).then(this.newTaskWindow).then(this.formTasksObj).then(this.addEvent).then(Tasks.hidePreloader).catch(this.errorHandler);
			}
		}, {
			key: 'checkToken',
			...
			key: 'changePageColors',
			value: function changePageColors(self) {

				return new Promise(function (resolve, reject) {

					var li = $('.user-nav a ul li'),
					    page = $('*');
			...
			key: 'eventChangeColor',
			value: function eventChangeColor(self) {
			...
			key: 'newTaskWindow',
			value: function newTaskWindow(self) {
			...
			key: 'editTaskWindow',
			value: function editTaskWindow(self) {
				...
				return new Promise(function (resolve, reject) {
					...
					icon.on('click', function (e) {
						...
						var wrapTask = e.target.closest('.task-content-wrap');
					});
					resolve(self);
				});
			}
			...
			key: 'formTasksObj',
			value: function formTasksObj(self) {
